...
- 403 permission error when pushing: you are likely authenticating as a different GitHub account (cached HTTPS credentials). Clear GitHub entries from Windows Credential Manager or switch to SSH as described above.
- `ssh -T git@github.com` shows `Permission denied`: ensure the public key was added to the correct GitHub account.

## Ready to commit?

If you want, I can stage and commit this README update for you. Pushing may still fail until your SSH is accepted by GitHub — verify with `ssh -T git@github.com` first.
...
```bash
ssh-keygen -t ed25519 -C "aje@gmail.com"
# Accept defaults (press Enter) to store at ~/.ssh/id_ed25519
```
